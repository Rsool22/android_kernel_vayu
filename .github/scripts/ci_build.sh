...
# ── ccache broken-pipe / LLVM ERROR fix ───────────────────────────────────────
# Root cause: when ccache falls back to PREPROCESSOR mode it runs `clang -E`
# through an internal pipe. Once it has the hash and finds a cache hit it
# closes the read end of the pipe. clang is still writing → EPIPE → each of
# clang's output-stream destructors fires "LLVM ERROR: IO failure on output
# stream: Broken pipe". The build still succeeds, but the messages are noisy.
#
# CCACHE_DIRECT=1      – primary fix: hash source files directly, no clang -E
#                        pipe at all for direct-mode hits.
# CCACHE_SLOPPINESS    – prevents the fallbacks that force preprocessor mode:
#   random_seed        : kernel passes -frandom-seed=<file> per TU; without
#                        this ccache treats each unique seed as a miss and
#                        falls back to preprocessor mode.
#   include_file_mtime : in CI a fresh git checkout gives all files the
#   include_file_ctime   current timestamp. ccache's direct-mode manifest
#                        stores the old mtime; without these flags ccache sees
#                        "mtime changed → manifest invalid → preprocess again".
#                        With them, ccache trusts content hashes over mtimes.
# CCACHE_BASEDIR       – normalises absolute paths before hashing so the cache
#                        is portable across different GitHub Actions runner
#                        machines (which may have different workspace paths).
# CCACHE_NOHASHDIR     – don't hash the CWD into cache keys; combined with
#                        BASEDIR this maximises cross-run cache hits.
...
# Stream the log live to the Actions console.
# grep filters the cosmetic "LLVM ERROR: IO failure on output stream: Broken pipe"
# messages that appear when ccache closes a preprocessor pipe early (non-fatal).
# The raw log file retains every line for post-build inspection.
tail -f "${BUILD_LOG}" | grep --line-buffered -v "^LLVM ERROR:" &
TAIL_PID=$!

"${MAKE[@]}" -j"$(nproc --all)" --output-sync=line > "${BUILD_LOG}" 2>&1
BUILD_STATUS=$?
