...
# CCACHE_DIRECT=1: skips the preprocessor pipe that ccache normally uses to
# hash source. Without it, ccache runs `clang -E` through an internal pipe
# and can stop reading early under parallel -j load → clang gets EPIPE →
# "LLVM ERROR: IO failure on output stream: Broken pipe".
export CCACHE_DIRECT="1"
# CCACHE_SLOPPINESS: kernel builds pass -frandom-seed=<file> per translation
# unit. Without random_seed sloppiness ccache treats each unique seed as a
# cache miss and re-runs the compiler through the preprocessor pipe, which
# re-introduces the broken-pipe risk on every cold-cache file.
